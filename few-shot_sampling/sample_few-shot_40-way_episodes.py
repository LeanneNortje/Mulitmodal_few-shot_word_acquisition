    ...
    for caption in entry['captions']:
        for word in vocab:
            if re.search(word, caption['text'].lower()) is not None and Path(caption['wav']).stem in alignments:
                if word in alignments[Path(caption['wav']).stem]:
                    if word not in val: val[word] = []
                    val[word].append((im, caption['wav'], caption['speaker']))

test_episodes = np.load(Path('../data/test_episodes.npz'), allow_pickle=True)['episodes'].item()
...
print(len(matching_set), len(test_episodes['matching_set']))
...
for word in vocab:
    ...
    for episode_num in tqdm(range(num_episodes)):
        ...
        if word not in test_episodes[episode_num]['queries']: test_episodes[episode_num]['queries'][word] = (entry[1], entry[2])
        if word not in test_episodes[episode_num]['matching_set']: test_episodes[episode_num]['matching_set'][word] = (entry[0])

for episode_n in range(num_episodes):
    if len(test_episodes[episode_num]['queries']) != 40 or len(test_episodes[episode_num]['matching_set']) != 40:
        print(len(test_episodes[episode_num]['queries']), len(test_episodes[episode_num]['matching_set']))
test_save_fn = '../data/40-way_test_episodes'
np.savez_compressed(
